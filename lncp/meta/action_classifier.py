#!/usr/bin/env python3
"""
QUIRRELY CLOSED-LOOP: ACTION CLASSIFIER v1.0-G2M
Classifies prescriptive actions into auto-apply vs human-review lanes.
G2M Release - 19 action domains.
...
class ActionDomain(str, Enum):
    ...
    # v3.1.1 domains
    GAMIFICATION = "gamification"       # Badge thresholds, XP rates, challenges
    PROGRESSIVE = "progressive"         # Unlock timing, day gates
    BUNDLING = "bundling"               # Bundle composition, savings display
    RETENTION = "retention"             # Churn intervention, offers
    ...
    # v5.1 SEO domains
    SEO = "seo"                         # Meta tags, descriptions, schema
    SEO_CONTENT = "seo_content"         # Content optimization, refresh
...
ALWAYS_HUMAN_DOMAINS: Set[ActionDomain] = {
    ...
    # v3.1.1: These require human review
    ActionDomain.BUNDLING,          # Pricing implications
    ActionDomain.TIER_PRICING,      # Revenue implications
}
...
AUTO_ELIGIBLE_DOMAINS: Set[ActionDomain] = {
    ...
    ActionDomain.LAYOUT,
    # v3.1.1: These can be auto-applied with constraints
    ActionDomain.GAMIFICATION,      # XP/badge adjustments
    ActionDomain.PROGRESSIVE,       # Unlock timing
    ActionDomain.NOTIFICATION,      # Notification triggers
    ActionDomain.SOCIAL_PROOF,      # Counter display
    # v5.1: SEO domains
    ActionDomain.SEO,               # Meta tags (with constraints)
}

# Partial auto-apply domains (some actions auto, some human)
PARTIAL_AUTO_DOMAINS: Set[ActionDomain] = {
    ActionDomain.RETENTION,         # Copy auto, discount offers human
    ActionDomain.SEO_CONTENT,       # Minor refresh auto, major restructure human
}
...
MAX_AUTO_CHANGE: Dict[ActionDomain, float] = {
    ...
    # v3.1.1 domains
    ActionDomain.GAMIFICATION: 20.0,    # Max 20% XP/threshold change
    ActionDomain.PROGRESSIVE: 15.0,     # Max ±1 day timing change
    ActionDomain.NOTIFICATION: 50.0,    # Max 50% timing change
    ActionDomain.SOCIAL_PROOF: 100.0,   # Can change display fully
    ActionDomain.RETENTION: 100.0,      # Copy only (offers are human)
    # v5.1 SEO domains
    ActionDomain.SEO: 100.0,            # Meta description changes
    ActionDomain.SEO_CONTENT: 50.0,     # Minor content refresh
    ActionDomain.SEO_TECHNICAL: 25.0,   # Technical SEO changes
}
...
MAX_AUTO_CHANGES_PER_DAY: int = 5  # Increased for v3.1.1
